...
{
    public static byte[] GenerarInterbancaria(DetalleOpi orden, ApiSettings settings)
    {
        const string title =
        ...
        var tableFirmas = FirmantesOrdenPago( orden.str_usuario_ingresa, orden.str_usuario_revisa,orden.str_usuario_aprueba );
        doc.Add( tableFirmas );
        doc.Close();
    ...
    public static byte[] GenerarOrdenPago(DetalleOpi orden, ApiSettings settings)
    {
        const string title =
        ...
        var cellInfroamcionPago = new Cell()
            .Add( new Paragraph( "OFICINA: " ).AddStyle( GetStyleReportes()["styleOrdenSubTitle"] ) )
            .AddStyle( styleCellOrdenValue );
        tableDetalle.AddCell( cellInfroamcionPago );
        cellInfroamcionPago = new Cell()

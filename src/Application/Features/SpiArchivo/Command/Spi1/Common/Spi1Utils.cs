...
{
    public static async Task<byte[]> GenerarSpi1Stream(CabeceraSpi1 cabeceraSpi1,
        IEnumerable<DetalleSpi1> lstDetalleSpi)
    {
        using var stream = new MemoryStream();
        await using var writer = new StreamWriter( stream );
        var cabeceraSpi1Txt =
            $"{cabeceraSpi1.dtt_fecha_envio:dd/MM/yyyy HH:mm:ss},{cabeceraSpi1.int_codigo_spi1},{cabeceraSpi1.int_num_total_opi},{cabeceraSpi1.dec_monto_total_opi},{cabeceraSpi1.int_num_control},{cabeceraSpi1.str_num_cuenta_io}";
    ...
    public static string CalcularValorMd5(byte[] data)
    {
        var hashBytes = MD5.HashData( data );

        var sb = new StringBuilder();

        foreach (var byteHashByte in hashBytes)
        {
            sb.Append( byteHashByte.ToString( "x2" ) );
        }
    ...
    public static RespuestaTransaccion ReporteConsolidado(ApiConfig config, string email, CabeceraSpi1 cabeceraSpi1, IReadOnlyList<DetalleSpi1Consolidado> listConsolidado, byte[] bytesSpi1)
    {
        ...
        var valorMd5 = CalcularValorMd5( bytesSpi1 );
